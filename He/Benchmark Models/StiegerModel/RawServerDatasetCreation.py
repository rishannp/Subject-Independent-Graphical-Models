...
def process_pkl_file(filepath):
    ...
    # Assuming file names are like "S1_Session_1" or "S10_Session_1"
    subject_token = filename.split('_')[0]  # e.g., "S1" or "S10"
    subject_id = int(subject_token.lstrip('S'))
    ...
    try:
        os.remove(filepath)
        print(f"[INFO] Deleted {filename} after processing.")
    except Exception as e:
