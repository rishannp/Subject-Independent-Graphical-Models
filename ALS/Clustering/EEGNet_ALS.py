...
from EEGModels import EEGNet  # Assuming EEGNet is defined in EEGModels.py
...
def load_and_preprocess_subject(subject_number, data_dir, fs, chunk_duration_sec=3):
    ...
    # For EEGNet, data should be shaped as (trials, channels, samples, 1)
    # Currently data is (trials, samples, channels) so we transpose it.
    data = data.transpose(0, 2, 1)  # Now (trials, channels, samples)
    data = data[..., np.newaxis]   # Add channel dimension: (trials, channels, samples, 1)
...
#%% LOSO EEGNet Training without Session Splitting
...
for test_subject in subject_numbers:
    ...
    # Determine EEGNet input dims
    _, chans, samples, _ = train_data.shape
    
    # Init and compile EEGNet
    model = EEGNet(
        nb_classes=2, Chans=chans, Samples=samples,
    ...
    # Train for all epochs without intermediate testing
    epochs = 25
    model.fit(
